...
class Cache : public MemoryBus
{
  public:
    ...
    Cache(std::size_t sets,
        ...
        : sets_{sets}, ways_{ways}, next_{std::move(next)}, policy_{wp}
    {
        data_.resize(sets_ * ways_);
        lru_way_.assign(sets_, 0);
    }
  ...
  private:
    ...
    std::vector<std::atomic_flag> lru_lock_{sets_};
...
{
    if (lru_lock_[set].test_and_set(std::memory_order_acquire)) return;
    lru_way_[set] = way;
